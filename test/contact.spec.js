...
describe("@Contacts", () => {
  ...
  it("Get alphabetically sorted list of contacts on /contacts?page=1&sortBy=firstName GET", done => {
    chai
      .request(app)
      ...
      .end((err, res) => {
        ...
        res.body.contacts.should.be.a("array");
        const contacts = res.body.contacts;
        const firstContact = contacts[0].firstName;
        const lastContact = contacts[contacts.length - 1].firstName;
        expect(firstContact.split("")[0] < lastContact.split("")[0]).to.equal(
          true
        );
        done();
      });
